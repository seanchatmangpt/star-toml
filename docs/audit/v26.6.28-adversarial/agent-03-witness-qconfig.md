# Audit Report: Witness & q_config (v26.6.28-adversarial)
...
## 1. Executive Summary
This report presents the audit of the cryptographic witness (`ConfigWitness`) and the admissibility standing indicator (`q_config`) of the `star-toml` library for the release tag `v26.6.28-adversarial`. The audit confirms that the configuration lifecycle follows strict typestate transitions, that the cryptographic witness deterministically binds all configuration provenance inputs to the canonical output byte representation, and that the conceptual admissibility bit `q_config` is lawfully governed.
...
- **Pipeline-Enforced Invariant**: The typestate pipeline ensures that `AdmittedConfig<T>` can *only* be created via [TrustedLoader::load_admitted](file:///Users/sac/star-toml/src/loader.rs#L1572-L1630) (or `load_admitted_strict`), which internally runs the complete validation sequence (`validate()`). If any stage fails (file missing, parse error, validation failure, unknown field in strict mode), the function returns an `Err(Error::...)` and halts.
...
1. **witness_missing_source_digest**
   ...
   - *Code*: Verified by check #18 in [verifier_report.rs](file:///Users/sac/star-toml/src/bin/verifier_report.rs#L289-L298).
...
2. **witness_missing_env_report**
   ...
   - *Code*: Verified by check #19 in [verifier_report.rs](file:///Users/sac/star-toml/src/bin/verifier_report.rs#L300-L312).
...
3. **witness_missing_validation_report**
   ...
   - *Code*: Verified by check #20 in [verifier_report.rs](file:///Users/sac/star-toml/src/bin/verifier_report.rs#L314-L324).
...
4. **witness_nondeterministic**
   ...
   - *Code*: Verified by check #21 in [verifier_report.rs](file:///Users/sac/star-toml/src/bin/verifier_report.rs#L326-L344) and `test_witness_is_deterministic` in [tests/brce.rs](file:///Users/sac/star-toml/tests/brce.rs#L423-L440).
...
6. **q_config true with failset_cardinality > 0**
   ...
   - *Mitigation*: The `verifier_report` binary exits with a non-zero exit code if any of the 23 detectors fail. Since the CI/CD pipeline enforces `failset_cardinality = 0` (all 23 checks passing), this falsifier cannot occur in the released tag.
...
| C | 23 counterexample checks in `src/bin/verifier_report.rs` | None |
...
| Metamorphic | [test_brce_metamorphic_canonical_stability](file:///Users/sac/star-toml/tests/brce.rs#L581-L598) | None | High |
...
| Idempotence | [test_brce_idempotence_canonical](file:///Users/sac/star-toml/tests/brce.rs#L600-L617) | None | High |
