...
3. **The Topology Bridge**:
   - Error fingerprinting (FNV variant hashing) is implemented in the `star_toml::validation` module, allowing production logs to emit stable variant IDs.
...
### 1. Error Topology and Variant Hashing (`star_toml::validation`)
- **FNV-1a Variant Hash**:
   - The runtime calculates a stable, inputs-independent `variant_id` representing the configuration failure pattern.
   - The variant hash is computed as the 64-bit FNV-1a hash of a pipe-joined (`|`) string containing sorted error path locations and their respective error codes:
     $$variant\_id = \text{FNV-1a}(\text{sorted}(\{\text{path\_location} + \text{":"} + \text{error\_code}\}) \text{ joined by } \text{"|"})$$
...
### 4. LSP Extension APIs for Project-Specific Checks (Deferred)
- **Extensible Validation Pipeline**:
   - Custom LSP extension providers (such as registering custom policy checks via public hooks/traits in `star-toml-lsp`) are deferred to a future release.
   - Document formatting is also deferred to a future release.

---

## Acceptance Criteria

- [x] **Stable Fingerprint Clustering**: Multiple independent parser executions with different concrete inputs (e.g., different invalid hostnames in a field, different out-of-bounds integers in another) that violate the exact same constraints at the same configuration paths yield identical `variant_id` hashes.
- [x] **Determinism in Sorting**: The FNV-1a hash calculation must sort error locations/codes prior to hashing to guarantee evaluation-order independence.
- [x] **Span Resolution**: For any configuration validation error (e.g., `MissingRequiredField`), the LSP can resolve the exact line/col span where the parent table is defined, or where the invalid type is specified.
- [x] **Duplicate Key Diagnostics**: Every occurrence of a duplicate key yields a diagnostic of severity `Error` showing the span of the duplicate key and references the span of the first definition.
- [x] **Resilience**: A TOML document with a syntax error at line 5 (e.g., missing quotes) still runs semantic checks on line 10, producing semantic diagnostics for line 10 if there are type/validation bounds violations.
- [x] **Unidirectional Dependency**: The `cargo tree` of the workspace confirms that the core `star-toml` library has zero dependencies on LSP crates.
- [x] **Extension Registration**: Custom LSP Extension Registration and Document Formatting are deferred.
...
- **LSP Extension Test**: LSP Extension and Formatting Tests are deferred.
